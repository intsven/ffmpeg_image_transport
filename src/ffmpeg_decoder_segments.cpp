...
FFMPEGDecoderSegments::FFMPEGDecoderSegments() : logger_(rclcpp::get_logger("FFMPEGDecoderSegments")) {}

FFMPEGDecoderSegments::~FFMPEGDecoderSegments() { reset(); }

void FFMPEGDecoderSegments::reset()
{
  if (codecContext_) {
...
bool FFMPEGDecoderSegments::initialize(
...
bool FFMPEGDecoderSegments::initDecoder(
...
bool FFMPEGDecoderSegments::decodePacket(const FFMPEGPacketConstPtr & msg)
{
  rclcpp::Time t0;
...
void FFMPEGDecoderSegments::resetTimers() { tdiffTotal_.reset(); }

void FFMPEGDecoderSegments::printTimers(const std::string & prefix) const
{
  RCLCPP_INFO_STREAM(logger_, prefix << " total decode: " << tdiffTotal_);
}

const std::unordered_map<std::string, std::string> & FFMPEGDecoderSegments::getDefaultEncoderToDecoderMap()
{
  return (defaultMap);
}  // need to fix includes instead
}  // namespace ffmpeg_image_transport
