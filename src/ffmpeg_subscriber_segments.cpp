...
FFMPEGSubscriberSegments::FFMPEGSubscriberSegments() : logger_(rclcpp::get_logger("FFMPEGSubscriberSegments")) {}

FFMPEGSubscriberSegments::~FFMPEGSubscriberSegments() {}

void FFMPEGSubscriberSegments::frameReady(const ImageConstPtr & img, bool) const { (*userCallback_)(img); }

#ifdef IMAGE_TRANSPORT_API_V1
void FFMPEGSubscriberSegments::subscribeImpl(
...
void FFMPEGSubscriberSegments::initialize(rclcpp::Node * node)
{
  node_ = node;

  // create parameters from default map
  for (const auto & kv : FFMPEGDecoderSegments::getDefaultEncoderToDecoderMap()) {
...
void FFMPEGSubscriberSegments::internalCallback(const FFMPEGPacketConstPtr & msg, const Callback & user_cb)
{
  if (!decoder_.isInitialized()) {
    ...
    if (!decoder_.initialize(
          msg, std::bind(&FFMPEGSubscriberSegments::frameReady, this, _1, _2), decoder)) {
